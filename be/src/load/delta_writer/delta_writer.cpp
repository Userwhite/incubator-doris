...
BaseDeltaWriter::BaseDeltaWriter(std::shared_ptr<WriteRequest> req, RuntimeProfile* profile,
                                 const UniqueId& load_id)
        : _req(std::move(req)), _memtable_writer(new MemTableWriter(*_req)) {
    _init_profile(profile);
}

DeltaWriter::DeltaWriter(StorageEngine& engine, std::shared_ptr<WriteRequest> req,
